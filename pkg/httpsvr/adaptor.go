...
import (
	...
	"github.com/dup2X/gomicrosvr/pkg/context"
	"github.com/dup2X/gomicrosvr/pkg/ctxutil"
	"github.com/dup2X/gomicrosvr/pkg/idgen"
...
func (ha *httpAdapt) getTimeout(r *http.Request) int64 {
	tw := r.Header.Get(context.DIDIRPCTimeoutMsKey)
	if tw == "" {
...
func setTrace(ctx stdctx.Context, r *http.Request) stdctx.Context {
	...
	traceID := r.Header.Get(context.DIDITraceIDKey)
	if traceID == "" {
		traceID = idgen.GenTraceID()
	}
	spanID := r.Header.Get(context.DIDISpanIDKey)
	if spanID == "" {
		spanID = idgen.GenSpanID()
	}
	hcode := r.Header.Get(context.DIDIHintCodeKey)
	hintCode, _ := strconv.ParseInt(hcode, 10, 64)
	hcontent := r.Header.Get(context.DIDIHintContentKey)
