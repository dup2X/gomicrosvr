...
import (
	...
	dctx "github.com/dup2X/gomicrosvr/pkg/ctxutil"
)

var (
	traceKey        = "#biz-lib-go.trace#"
...
func SetTrace(ctx stdctx.Context, r *http.Request) {
	hd := &Header{
		TraceID:     r.Header.Get(DIDITraceIDKey),
		SpanID:      r.Header.Get(DIDISpanIDKey),
		HintContent: r.Header.Get(DIDIHintContentKey),
	...
	hd.HintCode, _ = strconv.Atoi(r.Header.Get(DIDIHintCodeKey))
